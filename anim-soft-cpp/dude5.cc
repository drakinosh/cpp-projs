...
int mainLoop()
{
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::Closed) {
                ...
                if (event.key.code == sf::Keyboard::A) {
                    saveWindowToFile(cur_frame.text);
                } else if (event.key.code == sf::Keyboard::F) {
                    
                    sf::Vector2i pos = sf::Mouse::getPosition(window);
                    sf::Image img = cur_frame.text.copyToImage();
                    
                    floodFill(img, pos, tar_col, fill_col);
                    cur_frame.text.update(img);
                    /*
                ...
                mousedown = 0;
                cur_frame.text.update(window); // copy render target
                myVertices.clear();
            }
        ...
        sprite.setTexture(cur_frame.text);
